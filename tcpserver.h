...
namespace znb
{
	...
	class CNetSocketHelp{
		public:
			...
			static int initListenSockfd(uint32_t port, uint32_t ip = INADDR_ANY)
			{
                 ...
                 log(Info, "[initListenSockfd]:%u port:%u ok !!!!!!!", m_fd, port);
				 return m_fd;				
			}

			static CNetConn* createClientConn(const char* ip, uint32_t uport, CEpoll* pE, ILinkHandler* linkHandler, IDataHandler* dataHandler)
			{
				...
				CNetConn* pConn = new CNetConn(m_so, sa.sin_addr.s_addr, uport, linkHandler, dataHandler);
	...
	class TcpServer :	public CEvSource
					   ,public IDataHandlerAware
					   ,public ILinkHandlerAware
	{
		public:
			...
			explicit TcpServer(int fd):CEvSource(fd)
			{
			}
			~TcpServer()
			{
			...
			void startServer(uint32_t port);
