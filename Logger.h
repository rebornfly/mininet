...
{
    INTERFACE_LOG(const char* pInterface, const char* pModel, uint64_t uUid64, uint32_t uIp, uint32_t uUid, uint32_t uResStat, uint32_t uRequestId, uint32_t uTowardId)
        :interface(pInterface),module(pModel),uid64(uUid64),ip(uIp),uid(uUid),resStat(uResStat), requestId(uRequestId),towardId(uTowardId)
    {
    }
